...
FORCE_PREREQS_IN_CI="${FORCE_PREREQS_IN_CI:-0}"
WRITE_PROFILES="${WRITE_PROFILES:-1}"
RETRY_COUNT="${RETRY_COUNT:-3}"
PKG_MANAGER=""
if [ "$INSTALL_PREREQS" -eq 1 ] && { [ "$CI_DETECTED" -eq 0 ] || [ "$FORCE_PREREQS_IN_CI" -eq 1 ]; }; then
...
install_prereqs(){ case "$PKG_MANAGER" in
...
if [ "$INSTALL_PREREQS" -eq 1 ] && { [ "$CI_DETECTED" -eq 0 ] || [ "$FORCE_PREREQS_IN_CI" -eq 1 ]; }; then install_prereqs; fi
mkdir -p "$PYENV_ROOT/plugins"
clone_with_retries(){ local url="$1" dest="$2" i=0; while [ $i -lt "$RETRY_COUNT" ]; do if git clone --depth 1 "$url" "$dest" 2>/dev/null; then return 0; fi; i=$((i+1)); sleep $((i*2)); rm -rf "$dest"; done; return 1; }
...
if [ -d "$PYENV_ROOT" ]; then
  [ -f "$PYENV_ROOT/bin/pyenv" ] && chmod +x "$PYENV_ROOT/bin/pyenv" 2>/dev/null || true
  if [ ! -x "$PYENV_ROOT/bin/pyenv" ] && command -v pyenv >/dev/null 2>&1; then EXISTING_PYENV_PATH="$(command -v pyenv)"; EXISTING_ROOT="$(pyenv root 2>/dev/null || true)"; [ -n "$EXISTING_ROOT" ] && PYENV_ROOT="${PYENV_ROOT:-$EXISTING_ROOT}"; fi
fi
...
case "$DETECTED_SHELL" in
  ...
  *zsh*) [ "$WRITE_PROFILES" -eq 1 ] && _add_if_missing "$ZSH_PROFILE" 'export PYENV_ROOT="$HOME/.pyenv"' 'export PYENV_ROOT="$HOME/.pyenv"'; [ "$WRITE_PROFILES" -eq 1 ] && _add_if_missing "$ZSH_PROFILE" 'export PATH="$PYENV_ROOT/bin:$PATH"' 'export PATH="$PYENV_ROOT/bin:$PATH"'; [ "$WRITE_PROFILES" -eq 1 ] && _add_if_missing "$ZSH_PROFILE" 'eval "$(pyenv init --path)"' 'eval "$(pyenv init --path)"'; [ "$WRITE_PROFILES" -eq 1 ] && _add_if_missing "$ZSH_RC" 'export PYENV_ROOT="$HOME/.pyenv"' 'export PYENV_ROOT="$HOME/.pyenv"'; [ "$WRITE_PROFILES" -eq 1 ] && _add_if_missing "$ZSH_RC" 'export PATH="$PYENV_ROOT/bin:$PATH"' 'export PATH="$PYENV_ROOT/bin:$PATH"'; [ "$WRITE_PROFILES" -eq 1 ] && _add_if_missing "$ZSH_RC" 'eval "$(pyenv init -)"' 'eval "$(pyenv init -)"'; [ "$WRITE_PROFILES" -eq 1 ] && _add_if_missing "$ZSH_RC" 'eval "$(pyenv virtualenv-init -)"' 'eval "$(pyenv virtualenv-init -)"';;
  *bash*|*ksh*|*sh*|*dash*) if [ ! -f "$BASH_PROFILE" ] && [ -f "$PROFILE" ]; then BASH_PROFILE="$PROFILE"; fi; [ "$WRITE_PROFILES" -eq 1 ] && _add_if_missing "$BASH_PROFILE" 'export PYENV_ROOT="$HOME/.pyenv"' 'export PYENV_ROOT="$HOME/.pyenv"'; [ "$WRITE_PROFILES" -eq 1 ] && _add_if_missing "$BASH_PROFILE" 'export PATH="$PYENV_ROOT/bin:$PATH"' 'export PATH="$PYENV_ROOT/bin:$PATH"'; [ "$WRITE_PROFILES" -eq 1 ] && _add_if_missing "$BASH_PROFILE" 'eval "$(pyenv init --path)"' 'eval "$(pyenv init --path)"'; [ "$WRITE_PROFILES" -eq 1 ] && _add_if_missing "$BASH_RC" 'export PYENV_ROOT="$HOME/.pyenv"' 'export PYENV_ROOT="$HOME/.pyenv"'; [ "$WRITE_PROFILES" -eq 1 ] && _add_if_missing "$BASH_RC" 'export PATH="$PYENV_ROOT/bin:$PATH"' 'export PATH="$PYENV_ROOT/bin:$PATH"'; [ "$WRITE_PROFILES" -eq 1 ] && _add_if_missing "$BASH_RC" 'eval "$(pyenv init -)"' 'eval "$(pyenv init -)"'; [ "$WRITE_PROFILES" -eq 1 ] && _add_if_missing "$BASH_RC" 'eval "$(pyenv virtualenv-init -)"' 'eval "$(pyenv virtualenv-init -)"';;
  *) [ "$WRITE_PROFILES" -eq 1 ] && _add_if_missing "$PROFILE" 'export PYENV_ROOT="$HOME/.pyenv"' 'export PYENV_ROOT="$HOME/.pyenv"'; [ "$WRITE_PROFILES" -eq 1 ] && _add_if_missing "$PROFILE" 'export PATH="$PYENV_ROOT/bin:$PATH"' 'export PATH="$PYENV_ROOT/bin:$PATH"'; [ "$WRITE_PROFILES" -eq 1 ] && _add_if_missing "$PROFILE" 'eval "$(pyenv init --path)"' 'eval "$(pyenv init --path)"';;
esac
hash -r 2>/dev/null || true
oldpy="$PYENV_ROOT/bin"; tmp="$PATH"; IFS=':'; newp=""; for seg in $tmp; do [ "$seg" != "$oldpy" ] && newp="${newp:+$newp:}$seg"; done; unset IFS; PATH="$oldpy${PATH:+:}$newp"
export PATH="$PYENV_ROOT/bin:$PATH"
if [ -x "$PYENV_ROOT/bin/pyenv" ]; then
  set +e
  eval "$("$PYENV_ROOT/bin/pyenv" init --path)" 2>/dev/null || true
  eval "$("$PYENV_ROOT/bin/pyenv" init -)" 2>/dev/null || true
  if [ -d "$PYENV_ROOT/plugins/pyenv-virtualenv" ]; then eval "$("$PYENV_ROOT/bin/pyenv" virtualenv-init -)" 2>/dev/null || true; fi
  set -e
else
  ...
  else printf 'Warning: pyenv binary not found at %s. Clone may have failed or permissions prevent execution.\n' "$PYENV_ROOT/bin/pyenv" >&2; fi
fi
hash -r 2>/dev/null || true
if ! command -v pyenv >/dev/null 2>&1; then printf '\nError: pyenv not available after install. Check git, network, and permissions.\n' >&2; exit 1; fi
if [ "$CI_DETECTED" -eq 1 ] && [ -n "${GITHUB_ENV:-}" ]; then printf 'PYENV_ROOT=%s\n' "$PYENV_ROOT" >> "$GITHUB_ENV"; printf 'PATH=%s:$PATH\n' "$PYENV_ROOT/bin" >> "$GITHUB_ENV"; fi
