...
Write-Host "=== Engine Installation Script for Windows ===`nDetected OS: $os; Arch: $arch; Home: $userHome`n"
...
function Read-Input-Timeout($prompt,$default,$timeoutSec=30){
  try{ if(-not $Host.UI.RawUI){ return $default } } catch { return $default }
...
function Safe-WebContent($uri,$timeoutSec=15){
  try{ return (Invoke-WebRequest -Uri $uri -TimeoutSec $timeoutSec -ErrorAction Stop).Content } catch { try{ $wc=New-Object Net.WebClient; $wc.DownloadString($uri) } catch { return $null } }
}

$hasWinget = [bool](Get-Command winget -ErrorAction SilentlyContinue)
if(-not $hasWinget){ Write-Warning "winget not found. winget installs will be skipped." }

$pymgrCmd = $null; $pyCmd = $null
$g=Get-Command pymanager -ErrorAction SilentlyContinue; if($g){ $pymgrCmd = if($g.Path){ $g.Path } else { $g.Name } }
$g2=Get-Command py -ErrorAction SilentlyContinue; if($g2){ $pyCmd = if($g2.Path){ $g2.Path } else { $g2.Name } }

if(-not $pymgrCmd -and $hasWinget){
  Write-Host "Attempting winget install of PyManager..."
  $r=Run-Exec 'winget' @('install','--id','9NQ7512CXL7T','-e','--accept-source-agreements','--accept-package-agreements') 120
  if($r.Ok){ Start-Sleep -Seconds 1; $g=Get-Command pymanager -ErrorAction SilentlyContinue; if($g){ $pymgrCmd = if($g.Path){ $g.Path } else { $g.Name } } }
}
...
if($pyCmd){
  ...
  if($r.Ok -and $r.Out){ try{ $j=($r.Out -join "`n")|ConvertFrom-Json -ErrorAction Stop } catch{ $j=$null }; if($j){ foreach($it in $j){ if($it.tag){ $online += $it.tag } elseif($it.version){ $online += $it.version } elseif($it.name){ $online += $it.name } } } }
}
...
if($online.Count -eq 0){
  $t=Safe-WebContent 'https://www.python.org/ftp/python/' 12
  if($t){ $online=[System.Text.RegularExpressions.Regex]::Matches($t,'3\.\d+\.\d+') | ForEach-Object{$_.Value} | Sort-Object -Unique -Descending }
}
...
if($pyCmd){
  ...
  if($r.Ok -and $r.Out){
    try{ $inst=($r.Out -join "`n")|ConvertFrom-Json -ErrorAction Stop } catch{ $inst=$null }
    if($inst){ foreach($it in $inst){ if($it.tag){ $installed += $it.tag } elseif($it.version){ $installed += $it.version } } }
  } else {
...
if($latestPerMajor.Count -gt 0){ Write-Host "Latest (3.10+):"; "{0,-14} {1,-12}" -f "Version","Status" | Write-Host; foreach($v in $latestPerMajor){ $isInstalled=$installed -contains $v; "{0,-14} {1,-12}" -f $v,(if($isInstalled){"INSTALLED"}else{"Available"}) | Write-Host } } else { Write-Host "No online latest versions" }
...
if($installed -contains $SELECTED){ Write-Host "Python $SELECTED already installed; skipping install" } else {
  ...
  elseif($hasWinget){ Write-Host "Attempting winget install of a Python package as fallback..."; $pkg = "Python.Python.$($SELECTED.Split('.')[1])"; $r=Run-Exec 'winget' @('install','--id',$pkg,'-e','--accept-source-agreements','--accept-package-agreements') 600; if(-not $r.Ok){ Write-Warning "winget install failed or package id not available" } }
  else { Write-Warning "No installer available to install $SELECTED" }
}
...
Write-Host "`nOpen a new terminal to apply environment changes.`n"
